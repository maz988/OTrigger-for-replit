...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/admin/blog/generate", authenticateAdmin, async (req, res) => {
    try {
      ...
      // Always return generated content even if OpenAI API is not available
      // This ensures a better user experience during development or API issues
      ...
      // Try to use OpenAI if available, but fall back gracefully if not
      try {
        // Skip API call if key is not available or is default
        ...
        console.warn(`Using fallback content for blog generation due to: ${aiError.message}`);
        
        // Return the fallback content
        return res.status(200).json({
          success: true,
          data: fallbackPost,
          note: "Using fallback content due to OpenAI API unavailability"
        });
      }
    } catch (err: any) {
      console.error(`Error in blog generation endpoint: ${err.message}`);
      res.status(500).json({
        success: false,
        error: err.message
      });
